...
def Rotate():
	...
	try:
		...
		if auth == 200:
			...
			txt = "SELECT ss.description as Servizio, ss.description as Descrizione, ss.code as Codice, ssa2.code as Tipo, pp.value as Url, ss.id FROM ser_services ss Join ser_service_areas ssa2 on ss.id_area = ssa2.id join par_parameters pp on ss.id_url = pp.id JOIN ws_microservices wm on ss.id = wm.id_service where wm.code ='" + ws + "';" 
			servizi = query(txt)
			for s in servizi:
				url = s[4] + uri
				myobj = {'deg': deg, 'sender': chat}
				r = requests.post(url, json = myobj)
				t = generateAnswer('/ruota', r.status_code, s[5])
...
def SysParameters():
	...
	try:
		...
		if auth == 200:
			...
			txt = "select uu.username, ss.description, rr.description FROM usr_users uu JOIN ser_services ss on uu.id_service = ss.id JOIN rol_roles rr on uu.`role` = rr.id where uu.fl_deleted = 0	"
			utenti = query(txt)
